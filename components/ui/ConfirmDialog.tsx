 ...
 */
const DialogIcon = ({ iconBg, icon }: { iconBg: string; icon: React.ReactNode }) => (
...
const DialogFooter = ({ 
  ...
  <div className="px-6 py-4 bg-[#F9FBFF]/50 border-t border-[#F2F3F5] flex justify-end gap-3">
    ...
    <Button 
      ...
      className={cn("h-9 px-5 rounded-lg transition-all shadow-sm font-bold border", buttonStyle)}
    >
      {confirmText}
    </Button>
  </div>
      ...
      <div className="relative w-full max-w-[400px] bg-white rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.15)] overflow-hidden animate-in zoom-in-95 fade-in duration-200">
        <div className="p-6">
          <div className="flex items-start gap-4">
            <DialogIcon iconBg={style.iconBg} icon={style.icon} />
            {/* 内容区域 */}
