...
export default function Hero({ backgroundImage: initialBg, centerText, postMetadata }: HeroProps) {
  ...
  return (
    <section className="relative h-[480px] w-full overflow-hidden bg-slate-900">
      <HeroBackground bgUrl={bgUrl} imgLoaded={imgLoaded} />
      ...
      <div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center pb-10">
        <HeroContent postMetadata={postMetadata} centerText={centerText} />
      </div>
 ...
 * @param {string} props.bgUrl - 背景图 URL
 * @param {boolean} props.imgLoaded - 图片是否加载完成
 * @returns {JSX.Element}
 */
function HeroBackground({ bgUrl, imgLoaded }: { bgUrl: string; imgLoaded: boolean }) {
  return (
    <div 
 ...
 * Hero 内容组件
 * @param {Object} props - 组件属性
 * @param {PostMetadata} [props.postMetadata] - 文章元数据
 * @param {string} [props.centerText] - 居中展示的文字内容
 * @returns {JSX.Element}
 */
function HeroContent({ postMetadata, centerText }: { postMetadata?: PostMetadata; centerText?: string }) {
  ...
  return (
    <div className="mb-8 relative group cursor-default w-full max-w-5xl">
      {postMetadata ? (
        ...
        <h1 className="text-xl md:text-3xl font-bold tracking-[0.2em] px-4 py-2 flex items-center justify-center min-h-[80px] font-mono antialiased italic !text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
          {centerText}
        </h1>
      ) : (
        <h1 className="relative text-xl md:text-3xl font-bold tracking-[0.1em] font-mono min-h-[80px] flex items-center justify-center antialiased italic">
  ...
  return (
    <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] z-20">
      ...
      <style jsx>{`
        ...
        .animate-wave-slow { animation-delay: -2s !important; animation-duration: 7s !important; }
        .animate-wave-medium { animation-delay: -3s !important; animation-duration: 10s !important; }
        .animate-wave-fast { animation-delay: -4s !important; animation-duration: 13s !important; }
        @keyframes move-forever {
          0% { transform: translate3d(-90px,0,0); }
          100% { transform: translate3d(85px,0,0); }
        }
        @media (max-width: 768px) {
          .parallax > use { animation-duration: 10s; }
        }
      `}</style>
    </div>
  );
