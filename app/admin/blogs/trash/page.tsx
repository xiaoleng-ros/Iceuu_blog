...
import { Trash2, Inbox, CheckCircle2, XCircle, AlertCircle, ChevronLeft, ChevronRight, Calendar, ChevronDown, ChevronsLeft, ChevronsRight, RotateCcw, AlertTriangle } from 'lucide-react';
 ...
 * Toast 提示组件
 */
const Toast = ({ message, type = 'info', onClose }: { message: string; type?: 'success' | 'error' | 'info' | 'warning'; onClose: () => void; }) => {
  useEffect(() => {
  ...
  return (
    <div className={cn("fixed top-5 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-2 px-4 py-2.5 rounded-xl border shadow-lg animate-in fade-in slide-in-from-top-4 duration-300", bgColors[type])}>
      {icons[type]}
 ...
 * 自定义日期范围选择器组件
 */
const CustomDateRangePicker = ({ value, onChange, label }: { value: { start: string; end: string }; onChange: (val: { start: string; end: string }) => void; label: string; }) => {
  ...
  const renderCalendar = (monthDate: Date) => {
    ...
    return (
      <div className="w-[280px] p-4">
        <div className="text-center font-bold text-[#1D2129] mb-4 text-sm">{format(monthDate, 'yyyy年 M月', { locale: zhCN })}</div>
        <div className="grid grid-cols-7 gap-y-1">
          {weekDays.map(day => (<div key={day} className="text-center text-[#86909C] text-xs py-2">{day}</div>))}
          {calendarDays.map((day, idx) => {
            const isSelected = (value.start && isSameDay(day, parseISO(value.start))) || (value.end && isSameDay(day, parseISO(value.end)));
            const isInRange = value.start && value.end && isWithinInterval(day, { start: parseISO(value.start), end: parseISO(value.end) });
            const isCurrentMonth = isSameMonth(day, monthStart);
            return (
              <button key={idx} onClick={() => handleDateClick(day)} className={cn("h-8 w-full flex items-center justify-center text-xs transition-all relative", !isCurrentMonth ? "text-[#C9CDD4]" : "text-[#1D2129] hover:bg-[#F2F3F5] rounded-md", isSelected && "bg-[#165DFF] text-white rounded-md z-10 hover:bg-[#165DFF]", isInRange && !isSelected && "bg-[#E8F3FF] text-[#165DFF] rounded-none")}>
                {format(day, 'd')}
              </button>
            );
          })}
  ...
  return (
    <div className="flex items-center gap-2 relative w-full">
      ...
      <div className="relative w-full">
        <button type="button" onClick={() => setIsOpen(!isOpen)} className={cn("w-full h-8 rounded-lg border px-3 text-xs flex items-center justify-between transition-all bg-white", isOpen ? "border-[#165DFF] ring-1 ring-[#165DFF]" : "border-[#E5E6EB] hover:border-[#C9CDD4]")}>
          <div className="flex items-center gap-2 flex-1">
            <span className={cn("truncate", !value.start && "text-[#C9CDD4]")}>{value.start ? format(parseISO(value.start), 'yyyy/MM/dd') : "选择起始时间"}</span>
            <span className="text-[#C9CDD4]">→</span>
            <span className={cn("truncate", !value.end && "text-[#C9CDD4]")}>{value.end ? format(parseISO(value.end), 'yyyy/MM/dd') : "选择结束时间"}</span>
          </div>
            ...
            <div className="absolute top-full right-0 mt-2 bg-white border border-[#F2F3F5] rounded-xl shadow-[0_12px_32px_rgba(0,0,0,0.12)] z-50 overflow-hidden flex flex-col min-w-[600px]">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#F2F3F5]">
                <div className="flex gap-1">
                  <button onClick={() => setCurrentMonth(subMonths(currentMonth, 12))} className="p-1 hover:bg-[#F2F3F5] rounded text-[#86909C]"><ChevronsLeft className="w-4 h-4" /></button>
                  <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-1 hover:bg-[#F2F3F5] rounded text-[#86909C]"><ChevronLeft className="w-4 h-4" /></button>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-1 hover:bg-[#F2F3F5] rounded text-[#86909C]"><ChevronRight className="w-4 h-4" /></button>
                  <button onClick={() => setCurrentMonth(addMonths(currentMonth, 12))} className="p-1 hover:bg-[#F2F3F5] rounded text-[#86909C]"><ChevronsRight className="w-4 h-4" /></button>
                </div>
              </div>
              <div className="flex divide-x divide-[#F2F3F5]">{renderCalendar(currentMonth)}{renderCalendar(addMonths(currentMonth, 1))}</div>
...
const CustomSelect = ({ value, onChange, options, placeholder, label }: { value: string; onChange: (val: string) => void; options: string[]; placeholder: string; label: string; }) => {
  ...
  return (
    <div className="flex items-center gap-2 relative group w-full">
      ...
      <div className="relative w-full">
        <button type="button" onClick={() => setIsOpen(!isOpen)} className={cn("w-full h-8 rounded-lg border px-2 text-xs flex items-center justify-between transition-all bg-white", isOpen ? "border-[#165DFF] ring-1 ring-[#165DFF]" : "border-[#E5E6EB] hover:border-[#C9CDD4]", value ? "text-[#1D2129]" : "text-[#86909C]")}>
