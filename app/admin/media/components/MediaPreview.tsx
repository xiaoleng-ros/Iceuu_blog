  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in-95 duration-300">
        ...
        {/* 左侧：预览图 */}
        <PreviewImage url={item.url} filename={item.filename} />

        {/* 右侧：详细信息 */}
        <div className="w-full md:w-[360px] bg-white border-l border-[#F2F3F5] flex flex-col">
          <MediaHeader filename={item.filename} size={item.size} />
 ...
 * 媒体信息头部组件
 */
function MediaHeader({ filename, size }: { filename: string; size?: number }) {
  return (
    <div className="p-6 border-b border-[#F2F3F5]">
      <h3 className="text-lg font-bold text-[#1D2129] mb-1 truncate" title={filename}>
        {filename}
      </h3>
      <div className="flex items-center gap-2 text-xs text-[#86909C]">
        <span className="px-2 py-0.5 bg-[#F2F3F5] rounded-md font-medium uppercase">
          {filename.split('.').pop()}
        </span>
        <span>•</span>
